...
if __name__ == "__main__":
    print("=" * 72)
    print("PAPER 03 (corrected) — SHARED YIELD+SCHEDULE BUDGET, COSTLY YIELD")
    print("No inheritance, no boost. Blocked H burns budget on costly yields and")
    print("stops dominating. L is never starved (floor) and speeds up as H drains.")
    print("=" * 72)

    # --- with costly yield (the design) ---
    procs, S = build_share_scenario()
    s_costly = Scheduler(procs, verbose=False)
    L, H, M = procs
    status, rnd, stats = s_costly.run_share(costly_yield=True)
    print("\nCOSTLY YIELD (the design):")
    print(f"  status={status} @r{rnd}")
    print(f"  progress: {stats['progress']}")
    print(f"  final budgets: L={L.budget}, H={H.budget}, M={M.budget}")
    print(f"  round H's budget drops below L's (H burns down): {stats['burn_round']}")
    print(f"  round L's rate overtakes blocked H (speed-up):   {stats['speedup_round']}")

    # --- ablation: yield NOT costly (H never burns down) ---
    procs2, S2 = build_share_scenario()
    s_free = Scheduler(procs2, verbose=False)
    L2, H2, M2 = procs2
    status2, rnd2, stats2 = s_free.run_share(costly_yield=False)
    print("\nFREE YIELD (ablation: yield costs ~nothing):")
    print(f"  status={status2} @r{rnd2}")
    print(f"  round H burns down: {stats2['burn_round']} (expect later / None)")

    # --- claims ---
    # --- ablation A: floor on vs off (does L starve without the floor?) ---
    Lwin_floor, Lstall_floor, st_floor = measure_L_during_block(floor_frac=0.15)
    Lwin_nofloor, Lstall_nofloor, st_nofloor = measure_L_during_block(floor_frac=0.0)
    print("\n  --- ablation A: floor prevents starvation ---")
    print(f"    with floor (0.15): L made {Lwin_floor} progress during H's block, "
          f"stalled={Lstall_floor}, {st_floor}")
    print(f"    no floor   (0.00): L made {Lwin_nofloor} progress during H's block, "
          f"{st_nofloor}")

    # --- ablation B: costly vs free yield (does H dominate longer if yield free?) ---
    Lwin_costly, _, _ = measure_L_during_block(costly_yield=True)
    Lwin_free, _, _ = measure_L_during_block(costly_yield=False)
    print("\n  --- ablation B: costly yield makes H self-remove ---")
    print(f"    costly yield: L progress during block = {Lwin_costly}")
    print(f"    free yield  : L progress during block = {Lwin_free}")

    print("\n" + "=" * 72)
    print("VERDICT")
    print("=" * 72)
    L_progressed = stats['progress']['L'] > 0 and L.done
    H_burned = stats['burn_round'] is not None
    speedup_seen = stats['speedup_round'] is not None
    completes = status == "completed"
    floor_helps = (Lwin_floor >= Lwin_nofloor)   # floor gives L at least as much
    costly_helps = (Lwin_costly >= Lwin_free)     # costly yield frees L sooner
    print(f"  L never fully starved (made progress, completed):       {L_progressed}")
    print(f"  costly yield burned H's budget below L's (self-removal): {H_burned}")
    print(f"  L's rate overtakes blocked H (proportional speed-up):    {speedup_seen}")
    print(f"  floor gives L >= progress vs no floor:                   {floor_helps}")
    print(f"  costly yield gives L >= progress vs free yield:          {costly_helps}")
    print(f"  system completes:                                        {completes}")
    ok = (L_progressed and H_burned and speedup_seen and completes
          and floor_helps and costly_helps)
    print(f"\n  ALL CLAIMS HELD: {ok}")
    print("  (one shared budget; no inheritance; yield costly; floor prevents starvation.)")
